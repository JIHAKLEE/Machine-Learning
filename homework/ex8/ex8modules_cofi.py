...
def cofiCostFunc(params, Y, R, num_users, num_movies, num_features, lambda_var):
    ...
    J = (1/2)*np.sum(hypothesis**2) + (lambda_var/2)*np.sum(Theta**2) + (lambda_var/2)*np.sum(X**2)

    X_grad = np.dot(hypothesis, Theta) + lambda_var * X
    Theta_grad = np.dot(hypothesis.T, X) + lambda_var * Theta
